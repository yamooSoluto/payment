...
/**
 * 결제 스케줄 텍스트 생성 (배열 반환 - 각 항목이 별도 줄로 표시됨)
 */
export function getPaymentScheduleTexts(options: {
  amount: number;
  fullAmount?: number;
  ...
  if (isChangePlan && fullAmount) {
    ...
    if (isDowngrade && refundAmt > 0) {
      ...
      if (paymentAmt > 0) {
        return [
          `지금 ${formatPrice(paymentAmt)}원이 결제되고, 기존 플랜 미사용분 ${formatPrice(refundAmt)}원은 환불됩니다 (실 환불: ${formatPrice(netRefund)}원).`,
          `다음 결제일부터 매월 ${formatPrice(fullAmount)}원이 자동 결제됩니다.`,
        ];
      }
      return [
        `기존 플랜 미사용분 ${formatPrice(refundAmt)}원은 환불됩니다.`,
        `다음 결제일부터 매월 ${formatPrice(fullAmount)}원이 자동 결제됩니다.`,
      ];
    }
    ...
    if (refundAmt > 0) {
      ...
      return [
        `지금 ${formatPrice(paymentAmt)}원이 결제되고, 기존 플랜 미사용분 ${formatPrice(refundAmt)}원은 환불됩니다 (실 부담: ${formatPrice(netPayment)}원).`,
        `다음 결제일부터 매월 ${formatPrice(fullAmount)}원이 자동 결제됩니다.`,
      ];
    }
    return [
      `지금 ${formatPrice(paymentAmt)}원이 결제됩니다.`,
      `다음 결제일부터 매월 ${formatPrice(fullAmount)}원이 자동 결제됩니다.`,
    ];
  }
...
// 레거시 호환용 (단일 문자열 반환)
export function getPaymentScheduleText(options: Parameters<typeof getPaymentScheduleTexts>[0]): string {
  return getPaymentScheduleTexts(options).join(' ');
}
