...
export interface KeyDataSource {
  ...
  policyCategories?: string[]; // policy: 선택된 정책 카테고리 id 배열
}
...
export interface FaqTemplate {
  ...
  rule: string;
  policyRef?: string; // 정책 참조 키 (catId.respId) — handler 필드가 테넌트 정책과 일치할 때만 적용
  answerByHandler?: {
    bot?: string;
    staff?: string;
    staffHandler?: "op" | "manager";
  }; // policyRef 설정 시 handler별 분리 답변 (UI 전용, 저장 시 분리)
...
// ── policyRef handler 분리/병합 ──

// 로드 시: 같은 policyRef+questions를 공유하는 handler별 템플릿을 하나로 병합
...
// 저장 시: answerByHandler를 handler별 개별 템플릿으로 분리
    ...
    <div className="mt-3 pt-3 border-t border-stone-100">
      ...
      {availVars.length <= 1 && (
        <p className="text-[10px] text-gray-300 mt-1.5">
          우측 데이터 소스에서 섹션을 연결하면 해당 변수를 사용할 수 있습니다
        </p>
      )}
    </div>
  );
...
function PolicySourcePanel({
  ...
  const toggleCategory = (catId: string) => {
    ...
    // 카테고리 해제 시, 해당 카테고리의 policyRef도 해제
    ...
    <div className="space-y-2.5">
      <div>
        <label className="text-xs text-gray-500 font-medium">
          ...
          {selectedCatIds.length > 0 && (
            <span className="text-blue-500">({selectedCatIds.length}개 선택)</span>
          )}
        </label>
      ...
      {selectedCatIds.length > 0 && (
        <div className="space-y-2 pt-1">
          ...
          <p className="text-[10px] text-gray-400 -mt-1">특정 응답을 선택하면 점주 정책의 handler 설정이 이 템플릿에 연동됩니다</p>
      ...
      {selectedCatIds.length === 0 && (
        <p className="text-[11px] text-gray-400">포함할 정책 카테고리를 선택하세요</p>
      )}
    </div>
  );
}
        ...
        >
          ...
          <div className="flex gap-1.5 mb-3">
            {(["datasheet", "storeinfo", "policy"] as const).map((st) => (
              ...
              >
                {st === "datasheet" ? "데이터시트" : st === "storeinfo" ? "매장정보" : "이용규정"}
              </button>
            ))}
          </div>
          ...
          >
              ...
              <div className="flex items-center gap-3 px-5 py-3">
                ...
                <div className="ml-auto flex items-center gap-2.5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {!saving && hasDirty && (
                      <span className="text-[11px] text-amber-500">수정됨</span>
                    )}
            ...
            {filteredPackages.length === 0 && (
              <div className="text-center py-20 text-sm text-gray-400">
                {activeTab === "manual"
                  ? "패키지가 없습니다. 새 패키지를 추가하세요."
                  : "자동 FAQ 규칙이 없습니다. 새 규칙을 추가하세요."}
              </div>
            )}
                  ...
                  >
                    ...
                    <div className="flex items-center w-full px-5 py-3 border-b border-stone-100 rounded-t-2xl">
                      ...
                      >
                        ...
                        <span className="text-[11px] text-gray-400 tabular-nums">
                          {groupPkgs.length}개 패키지 · {groupFaqCount}건
                        </span>
                      </button>
                      <button
                        onClick={() => {
                          const groupTags =
                            groupKey === "공통" ? [] : groupKey.split(" · ");
                                              ...
                                              <div className="grid grid-cols-2 gap-6">
                                                  ...
                                                  t.answerByHandler ? (
                                                    <>
                                                      ...
                                                      <ChipEditor
                                                        ...
                                                        minRows={5}
                                                        placeholder="봇이 직접 안내할 때 사용할 답변"
                                                        onFocus={() => {
                                                      ...
                                                      <div className="flex items-center gap-2 mb-2 pl-1">
                                                        <span className="text-xs text-gray-500">전달 대상:</span>
                                                      ...
                                                      <div className="pl-1">
                                                        <span className="text-xs text-gray-500 mb-1 block">답변:</span>
                                                      </div>
                                                      <ChipEditor
                                                        value={
                                                          t.answerByHandler
                                                        ...
                                                        minRows={5}
                                                        placeholder="담당자에게 전달할 때 사용할 답변 (비우면 해당 handler 템플릿 미생성)"
                                                        onFocus={() => {
                                                          lastFocusedEditor.current =
                                                            "guide";
                                                        }}
                                                ...
                                                <div className="space-y-4">
                                                  ...
                                                  >
                                                    ...
                                                    {/* 전달 조건 (handler가 op/manager이고 policyRef 없을 때) */}
                                                    {!t.policyRef &&
                                                      (t.handler === "op" ||
                                                        t.handler ===
                                                          "manager") && (
                                                  ...
                                                  >
                                                    ...
                                                    <KeyDataSourceEditor
                                                      ...
                                                      onPolicyRefChange={(val) => {
                                                        ...
                                                        if (!val && t.answerByHandler) {
                                                          if (t.answerByHandler.staff?.trim() && !confirm("담당자 전달 답변이 삭제됩니다. 계속하시겠습니까?")) {
...
function RulesTab({
  ...
  const handleSyncDirty = useCallback(async () => {
    ...
    const syncLinkedFaqs =
      dirtyRules.length > 0 &&
      confirm("참조 중인 FAQ의 keyData도 함께 업데이트하시겠습니까?");
    try {
      await Promise.all(
  ...
  const handleDelete = useCallback(async (ruleId: string) => {
    try {
      ...
      if (res.status === 409 && data.linkedFaqCount) {
        if (
          !confirm(
            `이 규정을 ${data.linkedFaqCount}개 FAQ에서 참조 중입니다. 강제 삭제하시겠습니까?`
          )
        )
          return;
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-stone-100">
        <div>
          ...
          <span className="ml-2 text-xs text-gray-400">
            회원이 다양하게 표현하는 단어를 대표어로 치환하여 검색 정확도를
            높입니다.
          </span>
        </div>
        <div className="flex items-center gap-2">
